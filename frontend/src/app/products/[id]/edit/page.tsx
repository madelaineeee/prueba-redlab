...
export default function EditarProductoPage() {
  ...
  useEffect(() => {
    api.get<Producto>(`/productos/${id}`).then(({ data }) => {
      setNombre(data.nombre);
      setDescripcion(data.descripcion || "");
      setPrecio(data.precio.toString());
      setEstado(data.estado ? "true" : "false");
      setUsuarioCreacion(data.usuarioCreacion);
      setFechaCreacion(new Date(data.fechaCreacion).toLocaleDateString("es-ES"));
  ...
  if (cargando) return (
    <div className="flex min-h-screen bg-gray-50">
      <Sidebar />
      <main className="flex-1 flex items-center justify-center text-gray-400">Cargando...</main>
    </div>
  );
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 max-w-2xl">
          <h1 className="text-xl font-bold text-gray-800 mb-6">Editar producto</h1>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={usuarioCreacion} disabled
              ...
              <div>
                ...
                <input type="text" value={fechaCreacion} disabled
                  className="w-full border border-gray-100 bg-gray-50 rounded-lg px-3 py-2.5 text-gray-500 text-sm" />
              </div>
            </div>
